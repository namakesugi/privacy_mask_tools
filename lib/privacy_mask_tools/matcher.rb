...
module PrivacyMaskTools::Matcher
  ...
  MOBILE_NUMBER_REGEXP = /([(（]?([0０][5５7-9７-９][0０])[-ー()（）・ 　]*([0-9０-９]{4})[-ー()（）・ 　]*([0-9０-９]{4})(?![0-9０-９]))/

  # 携帯番号+IPフォン ハード判定用正規表現
  JARGON_MOBILE_NUMBER_REGEXP = /([(（]?([0０oO〇十]?[5５⑤五7-9７-９⑦-⑨七-九][0０十〇])[-ー()（）・ 　]*([0-9０-９oO①-⑨一-四〇壱弐参]{4})[-ー()（）・ 　]*([0-9０-９oO①-⑨一-四〇壱弐参]{4})(?![0-9０-９]))/

  # 固定電話 ソフト判定用正規表現
  PHONE_NUMBER_REGEXP = /([(（]?(([0０][3３4４6６])[-ー()（）・ 　]*([0-9０-９]{4})|([0０][1-9１-９]{2})[-ー()（）・ 　]*([0-9０-９]{3})|([0０][1-9１-９]{2}[0-9０-９])[-ー()（）・ 　]*([0-9０-９]{2})|([0０][1-9１-９]{2}[0-9０-９]{2})[-ー()（）・ 　]*([0-9０-９]))[-ー()（）・ 　]*([0-9０-９]{4})(?![0-9０-９]))/

  # 固定電話 ハード判定用正規表現
  JARGON_PHONE_NUMBER_REGEXP = /([(（]?(([0０oO〇十][3３三参③4４四④6６六⑥])[-ー()（）・ 　]*([0-9０-９oO①-⑨一-四〇壱弐参]{4})|([0０oO〇十][1-9１-９一-九四①-⑨壱弐参]{2})[-ー()（）・ 　]*([0-9０-９oO①-⑨一-四〇壱弐参]{3})|([0０oO〇十][1-9１-９一-九四①-⑨壱弐参]{2}[0-9０-９oO①-⑨一-四〇壱弐参])[-ー()（）・ 　]*([0-9０-９oO①-⑨一-四〇壱弐参]{2})|([0０oO〇十][1-9１-９一-九四①-⑨壱弐参]{2}[0-9０-９oO①-⑨一-四〇壱弐参]{2})[-ー()（）・ 　]*([0-9０-９oO①-⑨一-四〇壱弐参]))[-ー()（）・ 　]*([0-9０-９oO①-⑨一-四〇壱弐参]{4})(?![0-9０-９]))/
